...
if __name__ == "__main__":
    ...
    for i in range(number_of_tests):
        ...
        t1_object = datetime.strptime(t1, "%a %d %b %Y %H:%M:%S %z")
        t2_object = datetime.strptime(t2, "%a %d %b %Y %H:%M:%S %z")

        time_difference = t1_object - t2_object

        time_difference_seconds = abs(time_difference.total_seconds())

        print (f'{time_difference_seconds:.0f}')